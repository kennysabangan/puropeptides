    ...
    <div>
      ...
      <FadeSection className="py-20 md:py-28" style={{ backgroundColor: '#FAFAF7' }}>
        <div className="max-w-[1200px] mx-auto px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-16">
            {/* Processing */}
            <div>
              <h2 className="text-[clamp(1.5rem,3vw,2rem)] font-bold text-[#1A1F2E] tracking-[-0.02em] mb-6">
                Processing Times
              </h2>
              <div className="bg-white rounded-[20px] p-7 apple-shadow mb-6">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 bg-[#F7F5F0] rounded-full flex items-center justify-center">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#4CAF7D" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" />
                    </svg>
                  </div>
                  <div>
                    <h3 className="font-semibold text-[15px] text-[#1A1F2E]">0–2 Business Days</h3>
                    <p className="text-[13px] text-[#8B95A5]">Orders placed before 2 PM PST ship same day</p>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                {shippingMethods.map((m) => (
                  <div key={m.method} className="bg-white rounded-[20px] p-6 apple-shadow flex items-center justify-between">
                    <div>
                      <h3 className="font-semibold text-[15px] text-[#1A1F2E]">{m.method}</h3>
                      <p className="text-[13px] text-[#8B95A5]">{m.time}</p>
                    </div>
                    <span className="text-[13px] text-[#8B95A5]">{m.cost}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <FadeSection className="py-20 md:py-28">
        <div className="max-w-[1200px] mx-auto px-6 lg:px-8">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {[
              { title: 'Free Shipping Over $150', desc: 'Orders over $150 qualify for free standard shipping within the United States.', color: '#F7F5F0', icon: '🚚' },
      ...
      <FadeSection className="py-24" style={{ background: 'linear-gradient(135deg, #FFFDE7 0%, #F5F0FF 100%)' }}>
        <div className="max-w-[800px] mx-auto px-6 lg:px-8 text-center">
          <h2 className="text-[clamp(1.5rem,3.5vw,2.25rem)] font-bold text-[#1A1F2E] leading-snug tracking-[-0.02em] mb-8">
            Ready to order?
          </h2>
