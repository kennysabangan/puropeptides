  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-[18px] font-semibold text-[#1D1D1F]">Products</h2>
        <Link to="/admin/products/new" className="px-4 py-2 rounded-full bg-[#1D1D1F] text-white text-[12px] font-medium btn-apple">
          + New product
        </Link>
      </div>
            ...
            <tbody>
              {products.map((p) => (
                <tr key={p.id} className="border-t border-black/5">
                  ...
                  <td className="px-4 py-3 text-[#86868B]">{p.slug}</td>
                  <td className="px-4 py-3 text-right text-[#1D1D1F]">${Number(p.price).toFixed(2)}</td>
                  ...
                  <td className="px-4 py-3 text-right space-x-3">
                    <Link to={`/admin/products/${p.id}/edit`} className="text-[12px] text-[#1D1D1F] font-medium hover:underline">Edit</Link>
...
function Toggle({ on, disabled, onClick }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`relative inline-flex h-5 w-9 rounded-full transition ${on ? 'bg-[#34C759]' : 'bg-black/15'} ${disabled ? 'opacity-50' : ''}`}
    >
