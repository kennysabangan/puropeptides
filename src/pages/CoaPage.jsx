    ...
    <div>
          ...
          <div className="grid md:grid-cols-2 gap-5">
            {coaFields.map((item) => (
              <div key={item.field} className="bg-white rounded-[20px] p-7 apple-shadow card-lift">
                ...
                <h3 className="font-semibold text-[16px] text-[#1D1D1F] mb-2">{item.field}</h3>
          ...
          <div className="space-y-0">
            {testsPerformed.map((test, i) => (
              <div key={test} className="flex items-center gap-4 py-4 border-b border-[#E8E8ED] last:border-0">
                <div className="w-8 h-8 rounded-full bg-[#E8F5ED] flex items-center justify-center flex-shrink-0">
                ...
                <span className="text-[15px] text-[#1D1D1F]">{test}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <FadeSection className="py-24" style={{ background: 'linear-gradient(135deg, #FFFDE7 0%, #F5F0FF 100%)' }}>
        <div className="max-w-[800px] mx-auto px-6 lg:px-8 text-center">
          ...
          <p className="text-[#86868B] text-[15px] mb-8">Browse our catalog and see the quality for yourself.</p>
