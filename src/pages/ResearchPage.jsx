              ...
              <button
                ...
                className={`text-[13px] font-medium px-5 py-2.5 rounded-full transition-all ${
                  activeCategory === cat
                    ...
                    : 'bg-[#F5F5F7] text-[#1A1F2E] hover:bg-[#E8E8ED]'
                }`}
              >
                {cat}
              </button>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((article) => (
              <div key={article.title} className="bg-white rounded-[20px] p-7 apple-shadow card-lift">
                <span className="inline-block text-[11px] font-semibold text-[#8B95A5] tracking-wider uppercase mb-3">{article.category}</span>
                <h3 className="font-semibold text-[16px] text-[#1A1F2E] mb-2">{article.title}</h3>
                <p className="text-[13px] text-[#8B95A5] leading-relaxed mb-4">{article.desc}</p>
                <div className="flex items-center justify-between">
                  <span className="text-[12px] text-[#8B95A5]/60">{article.date}</span>
                  <span className="inline-flex items-center gap-1 text-[13px] font-medium text-[#1A1F2E] hover:gap-1.5 transition-all cursor-pointer">
                    Read more
