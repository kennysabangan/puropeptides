...
- Migrations to apply in order via Supabase SQL Editor:
  ...
  `004_storage_certificates.sql`, then `005_orders_payments.sql`
...
- Flow: Cart → `/checkout` (sign-up gated) → server creates an `orders`
  row and a NowPayments invoice → redirect to `invoice_url` →
